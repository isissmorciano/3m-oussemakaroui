...
def create_canzone(titolo: str, artista: str, durata: int) -> dict:
...
def mostra_playlist(playlist: dict) -> str:
    ...
    for i, canzone in enumerate(playlist["canzoni"], start=1):
        output += f"{i}. {info_canzone(canzone)}\n"
    totale = durata_totale(playlist)
    minuti = totale // 60
    secondi = totale % 60
